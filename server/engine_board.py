...
# --- Engine import (server-side flat layout) --------------------------------
try:
    import engine_core as _eng  # type: ignore
except Exception:  # pragma: no cover - dev/test fallback to the in-tree client copy
    try:
...
class DraftBoardState:
    ...
    def mirror(self, picks: Dict[str, Dict[str, str]],
        ...
        """Drive the board from an external authority (server snapshot / LCU
        live champ select). Replaces picks/bans/pointer wholesale.

        v3.0.5 chronology fix: history is now *incrementally* updated when the
        pointer advances (preserving the true action order observed locally),
        falling back to a role-order reconstruction only on cold-start /
        regression / desync. The previous behaviour always rebuilt history by
        iterating ROLES in order, which made `b.locked_at(action_idx)` return
        the wrong champion whenever picks happened out of ROLE order (e.g. B1
        was JGL, not TOP) — the timeline cells showed shuffled champions."""
        if our_side in SIDES:
            self.our_side = our_side
        new_picks = {"BLUE": dict(picks.get("BLUE", {})),
                     "RED":  dict(picks.get("RED", {}))}
        new_bans = {"BLUE": [c for c in bans.get("BLUE", []) if c],
                    "RED":  [c for c in bans.get("RED", []) if c]}
        new_pointer = max(0, min(int(completed), len(DRAFT_SEQUENCE)))

        prev_pointer = self.pointer
        # Validate that existing local history is still consistent with the
        # incoming snapshot (champs we recorded must still appear in picks/bans).
        history_still_valid = True
        for h in self._history:
            if h.kind == "ban":
                if h.champ not in new_bans.get(h.side, []):
                    history_still_valid = False
                    break
            else:
                if new_picks.get(h.side, {}).get(h.role) != h.champ:
                    history_still_valid = False
                    break
        ...
        if history_still_valid and new_pointer >= prev_pointer:
            # Advance: discover NEW actions between prev_pointer and new_pointer
            # by diffing against what's already in history.
            known_bans = {s: {h.champ for h in self._history
                              if h.kind == "ban" and h.side == s}
                          for s in SIDES}
            known_pickroles = {s: {h.role for h in self._history
                                   if h.kind == "pick" and h.side == s}
                               for s in SIDES}
            for a in DRAFT_SEQUENCE[prev_pointer:new_pointer]:
                if a.kind == "ban":
                    for c in self.bans.get(a.side, []):
                        if c not in known_bans[a.side]:
                            self._history.append(
                                _HistEntry(a.idx, "ban", a.side, c, None))
                            known_bans[a.side].add(c)
                            break
                else:
                    for r, c in self.picks.get(a.side, {}).items():
                        if r not in known_pickroles[a.side]:
                            self._history.append(
                                _HistEntry(a.idx, "pick", a.side, c, r))
                            known_pickroles[a.side].add(r)
                            break
            return

        # Cold-start / regression / desync — fall back to the role-order
        # reconstruction. Imperfect for cross-role pick orders but the best we
        # can do without an authoritative action log from the server. Once new
        # actions land, the incremental path above preserves their true order.
        self._history = []
        bi = {"BLUE": 0, "RED": 0}
    ...
    # --- serialization (Phase 2 cutover) -----------------------------------

    def to_dict(self) -> Dict[str, Any]:
        """JSON-safe snapshot of the entire state, for ferrying across the API."""
        return {
            "our_side": self.our_side,
            "pointer":  self.pointer,
            "players":  {s: [dict(p) for p in self.players[s]] for s in SIDES},
            "picks":    {s: dict(self.picks[s]) for s in SIDES},
            "bans":     {s: list(self.bans[s])  for s in SIDES},
            "history":  [{"action_idx": h.action_idx, "kind": h.kind,
                          "side": h.side, "champ": h.champ, "role": h.role}
                         for h in self._history],
        }

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "DraftBoardState":
        d = d or {}
        players = d.get("players") or {"BLUE": [], "RED": []}
        st = cls(
            blue_players=players.get("BLUE") or [],
            red_players=players.get("RED")  or [],
            our_side=str(d.get("our_side") or "BLUE"),
        )
        st.picks = {s: dict((d.get("picks") or {}).get(s, {})) for s in SIDES}
        st.bans  = {s: list((d.get("bans")  or {}).get(s, [])) for s in SIDES}
        st.pointer = int(d.get("pointer") or 0)
        st._history = [
            _HistEntry(int(h.get("action_idx", 0)), str(h.get("kind", "")),
                       str(h.get("side", "")), str(h.get("champ", "")),
                       h.get("role"))
...
def blind_safety(champ: str) -> float:
    ...
    v3.0.5: Champions with no entries in COUNTERS *and* no lane matchups now
    default to a neutral 0.5 instead of 1.0. The old behaviour treated table
    coverage holes as "perfect blind-safe", which over-weighted obscure
    champs in the early blind-pick branch (where blind_safe gets up to 22%
    of the final score)."""
    if _eng is None or not champ:
        return 0.5
    ...
    if c <= 0.0 and l <= 0.0:
        # No threat data on this champion in either table → "unknown", not
        # "perfectly safe". Apply only the flex bonus.
        safety = 0.5
    else:
        ...
        for role in open_r:
            ...
            for ch, sc in _candidates_for_player(
                ...
                if (is_counter_pick_slot and lane_known
                    ...
                    # Shifted to comfort (0.50 → 0.55) and steer (0.08 → 0.11)
                    # so the engine leans on real customs/scout data first.
                    score = (0.55 * cmf + 0.14 * bs + 0.18 * con
                             + 0.11 * steer + (0.04 if fr_unmatched >= 2 else 0.0))
                else:
